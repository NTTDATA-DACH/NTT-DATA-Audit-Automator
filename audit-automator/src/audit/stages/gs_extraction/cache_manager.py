...
class CacheManager:
    ...
    async def get_cached_result(self, kuerzel: str) -> Optional[Dict[str, Any]]:
        ...
        Reads straight away and treats NotFound as a miss: an exists-check first would
        be a second, blocking GCS round trip per Zielobjekt on the async path.
        """
    ...
    async def save_result_to_cache(self, kuerzel: str, result_data: Dict[str, Any]):
        ...
        cache_path = f"{INDIVIDUAL_RESULTS_PREFIX}{kuerzel}_result.json"
        try:
            await self.gcs_client.write_json_async(result_data, cache_path)
