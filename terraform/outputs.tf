...
output "next_step_gcloud_command" {
  ...
  value = "gcloud ai index-endpoints deploy-index ${google_vertex_ai_index_endpoint.bsi_audit_endpoint.name} --index=${google_vertex_ai_index.bsi_audit_index.name} --deployed-index-id=bsi-deployed-index-${var.customer_id} --display-name=bsi-deployed-index-${var.customer_id} --project=${var.project_id} --region=${var.region}"
}